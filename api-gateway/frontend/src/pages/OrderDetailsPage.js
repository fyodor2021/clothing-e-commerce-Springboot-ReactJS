...
import { useCancelOrderMutation, useLazyGetOrderByOrderIdQuery } from "../store";
...
export default function OrderDetailsPage() {
    ...
    if (orderDetailsResult.isSuccess && orderDetailsResult.data) {
      ...
      setRenderedProducts(
        orderDetailsResult.data.products.map((product, index) => {
          return (
            <div key={index} className="flex-row order-product-wrapper  bg-black bg-opacity-30 m-1">
              <div>
  ...
  return (
    orderDetails && <div className="mt-44 md:mt-36 xl:mt-48  py-10   w-full bg-black bg-opacity-80 font-semibold flex justify-center items-center xl:h-[calc(100vh-192px)]">
              ...
              <div>
                ...
                <div style={{ fontSize: "1rem" }}>
                  { orderDetails.status === "placed" ? (
                    ...
                    <span style={{ color: "red" }}>{ orderDetails.status}</span>
                  ) : (
          ...
          <div className="flex justify-center flex-col md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
            <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 dark:bg-gray-800 space-y-6">
              ...
              <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                <div className="flex justify-between items-center w-full">
                  ...
                  <p className="text-base dark:text-gray-300 leading-4 text-gray-600">
                    {
                      (orderDetails.orderTotal - orderDetails.orderTax).toFixed(
                        2
                      )}
                  </p>
                </div>
              ...
              <div className="flex justify-between items-center w-full">
                ...
                <p className="text-base dark:text-gray-300 leading-4 text-gray-600">
                  ${ orderDetails.totalPaidOnCard.toFixed(2)}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-gray-50 dark:bg-gray-800 w-full xl:w-96 flex justify-between  items-center md:items-start px-4 py-6 md:p-6 xl:p-8 flex-col">
          ...
          <div className="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
            ...
            <div className="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
              ...
              { orderDetails.status === "placed" ?
                            <button onClick={() => handleCancelOrder( orderDetails.orderId)} className="button w-full rounded-none shadow-none m-0 bg-white text-black">
                            { cancelLoading || isFetching ? <span className="animate-spin w-full"><AiOutlineLoading /></span> :"Cancel order"}
                            </button>
                        : <></>}
                <button
